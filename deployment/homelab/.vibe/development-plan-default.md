...
- **Dev server IPv4 binding + Vite host-check — clean two-line fix**: Alpine's `/etc/hosts` maps both `127.0.0.1` and `::1` to `localhost`; musl libc returns `::1` first, so Node dev servers bind IPv6-only and are unreachable via the pod's IPv4 address. Vite 5 also has a host-check middleware that blocks requests whose `Host` header isn't an IP or `localhost`. Fix: (1) `NODE_OPTIONS=--dns-result-order=ipv4first` pod env var — stable documented Node 16.4+ flag that makes `dns.lookup()` prefer IPv4 results, so `localhost` → `127.0.0.1` first; (2) `changeOrigin: true` on all `proxy.web()`/`proxy.ws()` calls in the router — http-proxy rewrites `Host` to the pod IP, and Vite's `host-validation-middleware` unconditionally allows IPv4 addresses. Note: `/etc/hosts` cannot be patched at build time (read-only bind-mount in Docker builds) nor at runtime by non-root (kubelet owns it). The `NODE_OPTIONS` flag is the only viable runtime fix.
