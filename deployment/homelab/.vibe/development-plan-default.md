...
- **Dev server IPv4 binding + Vite host-check — clean two-line fix**: Alpine's `/etc/hosts` maps both `127.0.0.1` and `::1` to `localhost`; musl libc returns `::1` first, so Node dev servers bind IPv6-only and are unreachable via the pod's IPv4 address. Vite 5 also has a host-check middleware that blocks requests whose `Host` header isn't an IP or `localhost`. Fix: (1) `sed -i '/^::1[[:space:]].*localhost/d' /etc/hosts` in the Dockerfile — localhost now resolves to `127.0.0.1` only; (2) `changeOrigin: true` on all `proxy.web()`/`proxy.ws()` calls in the router — http-proxy rewrites `Host` to the pod IP, and Vite's `host-validation-middleware` unconditionally allows IPv4 addresses. No monkey-patching, no ESM hooks, no `NODE_OPTIONS`.
