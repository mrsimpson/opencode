...
- **Dev server IPv4 binding + Vite host-check — final fix**: Two problems, two targeted fixes. (1) **Binding**: Kubernetes injects `::1 localhost` into `/etc/hosts`; musl libc returns `::1` first so `net.Server.listen(port, 'localhost')` binds IPv6 loopback — unreachable from other pods. Cannot fix `/etc/hosts` at build time (Docker bind-mount, read-only) or runtime as non-root (kubelet owns it, `644 root:root`). `--dns-result-order=ipv4first` makes localhost resolve to `127.0.0.1` but that's still loopback — still unreachable. Fix: `NODE_OPTIONS=--require /etc/bind-all-interfaces.cjs` loads a CJS patch that overrides `net.Server.prototype.listen` to replace any loopback address with `0.0.0.0`. Uses `--require` (not `--import`) so it works for both CJS and ESM entry points without ESM loader hooks. Verified: `listen(port, 'localhost')` → binds `0.0.0.0`. (2) **Vite host-check**: `changeOrigin: true` on `proxy.web()`/`proxy.ws()` in the router rewrites `Host` to the pod IP; Vite's `host-validation-middleware` unconditionally allows IPv4 addresses.
